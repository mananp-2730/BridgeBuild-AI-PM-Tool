...
"""
from prompts import get_system_prompt, get_sales_prompt
        ...
        if saved_tickets:
            for i, item in enumerate(saved_tickets):
                with st.expander(f"Ticket: {item['summary'][:60]}..."):
                    ...
                    # --- PREPARE FULL EMAIL CONTENT FOR HISTORY FIRST ---
                    ...
                    # --- 2. INDIVIDUAL HISTORY ACTIONS (Rendered ONCE) ---
...
def sales_dashboard():
    ...
    # Grab the user's settings (from the database profile we set up yesterday!)
    api_key = st.secrets.get("GOOGLE_API_KEY")
    ...
    if st.button("Analyze Request", type="primary"):
        if not api_key:
            ...
            try:
                ...
                with col1:
                    st.metric("Estimated MVP Timeline", data.get("estimated_timeline"))
                with col2:
                    st.metric("Estimated MVP Budget", f"{fmt_low} - {fmt_high}")
                    
                st.divider()
                
                # Questions & Deal Breakers
