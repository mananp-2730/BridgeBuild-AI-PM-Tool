...
# ==========================================
# TICKET 2: UNIVERSAL EXPORT ENGINE
# ==========================================
def render_universal_exports(data, role_name, currency="USD ($)"):
    """
    Creates a standardized PDF download and Email share button 
    that can be plugged into ANY dashboard (Sales, Design, Eng, PM).
    """
    st.divider()
    ...
    with col_action1:
        ...
        try:
            # We wrap this in a try/except just in case the PDF generator encounters missing data
            pdf_data = create_pdf(data, currency, ticket_type="brief") 
            st.download_button(
    ...
    with col_action2:
        ...
        body = f"Hello Team,\n\nPlease review the latest {role_name} requirements generated by BridgeBuild AI...\n\n-> SUMMARY:\n{data.get('summary', 'See attached PDF for details.')}\n\nBest,\nBridgeBuild Platform"
        mailto_link = f"mailto:?subject={urllib.parse.quote(f'{role_name} Update: {ticket_name}')}&body={urllib.parse.quote(body)}"
        
        # We use a custom HTML button so it matches the Streamlit UI but acts as a mailto link
        st.markdown(f"""
