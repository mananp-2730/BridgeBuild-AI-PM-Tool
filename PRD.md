...
3. **Handoff & Negotiation:** The ticket is routed to the PM Hub, where the AI breaks the approved scope into Agile Epics and User Stories. If the client pushes back on the price, the PM uses the interactive **Scope-Slider** to dynamically downgrade the architecture and push non-essentials to Phase 2. The PM can also use **God-Mode** to directly edit raw JSON elements.
