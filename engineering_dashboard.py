...
def provision_github_repo(token, repo_name, eng_data, frontend_code):
    ...
    try:
        ...
        # 2. Sanitize Repo Name (GitHub allows alphanumeric and hyphens)
        ...
        repo = user.create_repo(
            ...
            auto_init=True # Auto-creates a main branch with a blank README
        )
        ...
        readme_content += f"- **Frontend:** {stack.get('frontend', 'N/A')}\n"
        readme_content += f"- **Backend:** {stack.get('backend', 'N/A')}\n"
        readme_content += f"- **Database:** {stack.get('database', 'N/A')}\n"
        
        # 5. Commit the README update
        contents = repo.get_contents("README.md")
        repo.update_file(contents.path, "Initial Commit: Architecture README via BridgeBuild AI", readme_content, contents.sha, branch="main")
        
        # (We will add the SQL Schema and React code commits in Step 2!)
