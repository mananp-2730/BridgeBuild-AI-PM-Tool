...
# ==========================================
# GITHUB ZERO-TO-REPO ENGINE
# ==========================================
def provision_github_repo(token, repo_name, eng_data, frontend_code):
    """Authenticates with GitHub, creates a private repo, and commits initial files."""
    try:
        # 1. Authenticate & Create Repo
        auth = Auth.Token(token)
        g = Github(auth=auth)
        user = g.get_user()
        
        clean_name = re.sub(r'[^a-zA-Z0-9-]', '-', repo_name.lower()).strip('-')
        
        repo = user.create_repo(
            name=clean_name,
            private=True,
            description="Auto-generated by BridgeBuild AI Enterprise OS",
            auto_init=True 
        )
        
        # 2. Commit README.md
        readme_content = f"# {clean_name.capitalize()}\n\n"
        readme_content += f"## System Architecture\n{eng_data.get('system_architecture', 'N/A')}\n\n"
        stack = eng_data.get('tech_stack_recommendation', {})
        ...
        repo.update_file(contents.path, "docs: add architecture README", readme_content, contents.sha, branch="main")
        
        # 3. Commit Database Schema (schema.sql)
        sql_content = "-- Auto-generated PostgreSQL Schema by BridgeBuild AI\n\n"
        for table in eng_data.get("database_schema", []):
            t_name = table.get("table_name", "unknown_table").replace(" ", "_").lower()
            sql_content += f"CREATE TABLE {t_name} (\n"
            cols = []
            for col in table.get("columns", []):
                cols.append(f"    {col}")
            sql_content += ",\n".join(cols) + "\n);\n\n"
            
        repo.create_file("schema.sql", "db: add database schema", sql_content, branch="main")
        
        # 4. Commit React Frontend Code (If Inherited from Design Hub)
        if frontend_code and "generated_components" in frontend_code:
            for comp in frontend_code.get("generated_components", []):
                comp_name = comp.get("component_name", "Component").replace(" ", "")
                comp_code = comp.get("code", "// Component code missing")
                # Commits into a dedicated React folder structure!
                repo.create_file(f"src/components/{comp_name}.jsx", f"feat: add {comp_name} UI component", comp_code, branch="main")

        return repo.html_url, None
